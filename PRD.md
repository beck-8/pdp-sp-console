...
9. **Alerts are loud, but only for real problems.** The dashboard's open-issues list shows final failures and active conditions only — a task attempt that later succeeds on retry never appears there. The machinery is the alert lifecycle proposed in curio PR #1332 (open at time of writing: `alert_conditions` for active conditions; `alert_history.kind = event|condition` for one-shots and resolved history; `alert_mutes` is already on main) — the console reads it, it doesn't reinvent it. One gap to close when wiring up: that PR's task-failure query still counts every failed attempt (`result = FALSE` over an interval), so the final-failure filter (retries exhausted — the task row is deleted from `harmony_task`) has to be added there or in our read RPC. Recovered attempts stay visible one level down, on Storage & Tasks, for whoever wants to chase an underlying cause. (Decided 2026-07-07.)
...
| Open-issues list | `alert_conditions` (active conditions: system/subsystem, message, `created_at`, `repeat_count`) + recent `alert_history` rows with `kind='event'`, plus the existing mute mechanism. Final failures and active conditions only — retried-and-recovered attempts are filtered out (core decision 9). Empty list ⇒ the banner reads "All clear". Rows deep-link to the page that owns the problem. | 🔶 read RPC over the alert-lifecycle tables (PR #1332, open) |
